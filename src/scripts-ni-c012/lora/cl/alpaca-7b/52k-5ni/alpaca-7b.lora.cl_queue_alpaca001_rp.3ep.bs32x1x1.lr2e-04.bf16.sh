...
CKPT_DIR=$REPO_ROOT_DIR/saves/ni-c012/ALPACA-7B/lora/cl_queue_alpaca001_qa/bs32x1x1-3ep-bf16
...
do
    ...
    CKPT_DIR=$REPO_ROOT_DIR/saves/ni-c012/ALPACA-7B/lora/cl_queue_alpaca001_rp${SMP_RATIO}_${CUR_CATE}/bs32x1x1-3ep-bf16
    if [ -d $CKPT_DIR ]; then
        echo File $CKPT_DIR exists.
        continue
    fi

    DATASETS="alpaca_en_smp001,ni_c012_${CUR_CATE}_train"
...
do
    ...
    then
        ...
        CKPT_DIR=$REPO_ROOT_DIR/saves/ni-c012/ALPACA-7B/lora/cl_queue_alpaca001_${CUR_CATE}/bs32x1x1-3ep-bf16
    fi
    ...
    do
        ...
        if [ -f $CKPT_DIR/ni_c012_${CUR_CATE_Y}_eval/all_results.json ]; then
            ...
            echo $CKPT_DIR/ni_c012_${CUR_CATE_Y}_eval/all_results.json does not exist.
        fi
        CUDA_VISIBLE_DEVICES=$CUDA python3 $SRC_DIR/train_bash.py \
            --stage sftrp \
            --model_name_or_path $MODEL_DIR \
