...
CKPT_DIR=$REPO_ROOT_DIR/saves/ni-c012/ALPACA-7B/lora/cl_queue_alpaca${SMP_RATIO}_${CATE}/bs32x1x1-3ep-bf16
if [ -d $CKPT_DIR ]; then
    echo File $CKPT_DIR exists.
    continue
fi

DATASETS="alpaca_en_smp${SMP_RATIO},ni_c012_${CATE}_train"
...
do
    if [ -d $CKPT_DIR/ni_c012_${CUR_CATE}_eval ];
    then
        echo Directory $CKPT_DIR/ni_c012_${CUR_CATE}_eval exists.
    else
        echo $CUR_CATE
        CUDA_VISIBLE_DEVICES=$CUDA python3 $SRC_DIR/train_bash.py \
            --stage sftrp \
